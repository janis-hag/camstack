...
class NUVU(EDTCamera):
    ...
    INTERACTIVE_SHELL_METHODS = [
        'GetReadoutMode','GetReadoutTime',
        'GetExposureTime', 'SetExposureTime',
        'SetCCDTemperature', 'GetCCDTemperature',
        'GetEMCalibratedGain', 'SetEMCalibratedGain',
        'GetEMRawGain', 'SetEMRawGain',
        'GetAnalogicGain', 'SetAnalogicGain',
        'FULL',
        ] + \
    ...
    MODES = {
            # FULL 128 x 128
            FULL:
                    CameraMode(x0=0, x1=127, y0=0, y1=127),
            0:
                    CameraMode(x0=0, x1=127, y0=0, y1=127, fps=1500.,
                               tint=0.0006),
            1:
                    CameraMode(x0=0, x1=520, y0=0, y1=70, fps=1500.,
                               tint=0.0006, fgsize=(520, 70)),
    }
    ...
    def _get_nuvu_response(self, response, verbose=0):
        ...
        try:
            ...
            try:
                ...
                #pass
                if 3 == len(rlines) and not ':' in rlines[0]:
    ...
    def _update_nuvu_config(self, retries: int = 10, timeout: float = 100.):
        ...
        while r < retries:
            ...
            return None
        (success, resdict) = self._get_nuvu_response(resp)
        logging.debug("Communication initialised with ld 0")
    ...
    def SetReadoutModeStr(self, romode):
        if not romode in self.RO_MODES:
            return False
        (success,
         resdict) = self.send_command("ld %d" % (self.RO_MODES.index(romode)),
                                      timeout=400.)
        if success:
    ...
    def _update_romodes_list(self):
        ...
        if success:
            for i in range(len(resdict)):
                self.RO_MODES.append(resdict[str(i)].split()[0])
        return success
    ...
    def GetCtrlTemperature(self):
        (success, answer) = self.send_command("{cmd}".format(
                cmd=self.cfgdict['GetTempCtrlCmd']))
        if success:
    ...
    def GetCCDTemperature(self):
        (success, answer) = self.send_command("{cmd}".format(
                cmd=self.cfgdict['GetTempCCDCmd']))
        if success:
            temp = float(answer['1'])
    ...
    def GetSetCCDTemperature(self):
        (success, answer) = self.send_command("{cmd}".format(
                cmd=self.cfgdict['GetSetTempCCDCmd']))
        if success:
    ...
    def GetEMRawGain(self):
        (success, answer) = self.send_command("{cmd}".format(
                cmd=self.cfgdict['EMGetRawGainCmd']))
        if success:
    ...
    def GetAnalogicGain(self):
        (success, answer) = self.send_command("{cmd}".format(
                cmd=self.cfgdict['AnalogicGetGainCmd']))
        if success:
    ...
    def GetAnalogicOffset(self):
        (success, answer) = self.send_command("{cmd}".format(
                cmd=self.cfgdict['AnalogicGetOffsetCmd']))
        if success:
